...
# Config 
class Config(object):
    BOT_TOKEN = os.environ.get("BOT_TOKEN", "6907561694:AAE6WcG-uOtkpQrjgl8bdlk2hVD4URFgxHI")
    API_ID = int(os.environ.get("API_ID", "19718025"))
    API_HASH = os.environ.get("API_HASH", "304ccfbb51904423c374a65c31bf7e50")
    DOWNLOAD_LOCATION = "./DOWNLOADS"
    SESSIONS = "./SESSIONS"

    AUTH_USERS = os.environ.get('AUTH_USERS', '5223390523').split(',')
    for i in range(len(AUTH_USERS)):
        AUTH_USERS[i] = int(AUTH_USERS[i])

    GROUPS = os.environ.get('GROUPS', '-1001978613922').split(',')
    for i in range(len(GROUPS)):
        GROUPS[i] = int(GROUPS[i])

    LOG_CH = -1002134928259
